...
function Rig({ layout, height }: { layout: SetLayout; height: number }) {
  ...
  // A drop is a small scene: the camera eases in while the brick falls and eases back
  // out after it lands. Smooth, so only the brick moves on stop-motion steps.
  useEffect(() => {
    const c = controls.current
    if (!c || !flying || film) return
    const home = c.distance
    void c.dollyTo(home * 0.86, true)
    invalidate()
    const back = window.setTimeout(() => {
      void c.dollyTo(home, true)
      invalidate()
    }, 1100)
